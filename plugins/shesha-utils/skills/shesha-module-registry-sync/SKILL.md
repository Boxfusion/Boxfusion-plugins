...
- **All file reads and outgoing HTTP bodies must be explicit UTF-8, never left to PowerShell's defaults.** On Windows PowerShell 5.1, `Get-Content` without `-Encoding UTF8` misreads a BOM-less UTF-8 file (the common case for `.json`/`.cs` written by modern tooling) using the system codepage instead, turning non-ASCII characters (an em-dash, curly quotes) into mojibake — and separately, `Invoke-RestMethod -Body <string>` encodes the body using the system codepage's lossy "best fit" table (silently downgrading an em-dash to a plain hyphen) rather than real UTF-8, unless the body is passed as an explicit UTF-8 byte array (`[System.Text.Encoding]::UTF8.GetBytes(...)`) with `-ContentType "application/json; charset=utf-8"`. Both were real bugs here (confirmed live, not just theorized) — the Update/Insert/Authenticate calls now all encode explicitly, and every `Get-Content` call passes `-Encoding UTF8`. This is exactly why a payload that fails from the script can look byte-for-bit "the same" as one pasted into Swagger and yet behave differently — Swagger sends real UTF-8; the script, without these fixes, didn't.
