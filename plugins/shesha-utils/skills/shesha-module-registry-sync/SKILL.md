...
6. Query the private npm registry doc for the matched package (one call returns every version + dependencies + description + a `time` map of version → publish timestamp), applying the same dependency filter and reading `publishedDate` straight out of that `time` map.
